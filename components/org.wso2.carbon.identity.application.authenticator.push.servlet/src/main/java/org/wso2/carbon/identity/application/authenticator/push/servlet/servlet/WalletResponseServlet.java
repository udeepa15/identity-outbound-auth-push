...
/**
 * Servlet for handling wallet response requests.
 */
public class WalletResponseServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (StringUtils.isEmpty(vpToken) || StringUtils.isEmpty(state)) {
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            response.setContentType("application/json");
            PrintWriter out = response.getWriter();
            out.print("{\"error\": \"Missing required parameters: vp_token and state\"}");
            out.flush();
        } else {
